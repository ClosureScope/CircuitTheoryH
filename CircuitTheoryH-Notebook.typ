...
=== 复功率
+ 复功率
  $ tilde(S) = P + j Q = U I angle phi = dot(U) dot(I)^* = I^2 Z = U^2 / Z^* $
...
+ 正弦稳态电路——负载电阻和电抗均可独立变化
  ...
  最大有功功率：$p = abs(u_(o c))^2 / (4 "Re"Z_L)$
...
+ 正弦稳态电路—— 负载阻抗角固定而模可改变
  ...
  最大有功功率：$p = (abs(u_(o c))^2 cos phi_L )/ (2 abs(Z_L) + 2 (R_o cos phi_L + X_o sin phi_L))$
...
+ 三相电路的功率因数
  ...
  *对称三相电路*
  $ cos phi^' = cos phi_P $
  #note[额定功率][额定功率为$P_N$，有功功率为$P$，额定效率为$eta_N$ $ P_N/P = eta_N $]
=== 三相电路功率的测量
