...
+ 戴维宁定理
  #theorem[戴维宁定理][
    ...
    任何线性含源电阻电路$N$，就其两个端钮而言，总可以用一个独立电压源$u_(o c)$与一个电阻$R_(e q)$的串联组合来等效。其中，电压源的电压$u_(o c)$等于该电路$N$的开路电压，即电路$N$不接负载时两个端钮间电压；电阻$R_(e q)$为该电路$N$中全部独立电源置零后所得无源电路$N_0$的等效电阻。
  ]
  #note[][
...
+ 等效电阻的求取
  ...
  + 开路电压和短路电流法（不除源）
  + 加接测试电阻法（不除源）
  #note[][1最简便；在有受控源时，一般只得采用2或3；4在特殊情况下采用。]

== 诺顿定理
#theorem[诺顿定理][

  任何线性含源电阻电路$N$，就其两个端钮而言，总可以用一个独立电流源$i(s c)$与一个电导$G_(e q)$的并联组合来等效。其中，电流源的电流$i_(s c)$等于该电路$N$的短路电流；并联电导$G_(e q)$为该电路中所有独立电源置零后所得无源电路$N_0$的等效电导。
]
#note[][
...
+ 互易定理1
  #theorem[互易定理1][
    ...
    对内部不含独立源和受控源的线性电阻电路$N_0$，任取两对端钮$alpha alpha^'$和$beta beta^'$，如果在端口$alpha alpha^'$施加输入电压$u_(s alpha)$在端口$beta beta^'$可得到输出电流$i_beta$。反之，对$beta beta^'$施加输入电压$u_(s beta)^hat$，可在$alpha alpha^'$得到输出电流$i_alpha^hat$，则有
    $ i_alpha^hat / u_(s beta)^hat = i_beta / u_(s alpha) $
    若$u_(s beta)^hat = u_(s alpha)$，则$i_alpha^hat = i_beta$
    #image("assets/image-39.png")
  ]

  #note[][
    + 互易前后端口处的激励和响应的极性保持一致（要么都一致，要么都不一致）。
    + 互易定理只能求出互易支路的电压、电流，其他支路的电压、电流发生变化。
    + 互易定理用于解平衡电桥网络和对称网络较方便。
  ]
  #theorem[电桥平衡条件][$ R_1 R_4 = R_2 R_3 $]
+ 互易定理2
  #theorem[互易定理2][

    对内部不含独立源和受控源的线性电阻电路$N_0$，任取两对端钮$alpha alpha^'$和$beta beta^'$，如果在端口$alpha alpha^'$施加输入电流$i_(s alpha)$在端口$beta beta^'$可得到输出电压$u_beta$。反之，对$beta beta^'$施加输入电流$i_(s beta)^hat$，可在$alpha alpha^'$得到输出电压$u_alpha^hat$，则有
    $ u_alpha^hat / i_(s beta)^hat = u_beta / i_(s alpha) $
    若$i_(s beta)^hat = i_(s alpha)$，则$u_alpha^hat = u_beta$
